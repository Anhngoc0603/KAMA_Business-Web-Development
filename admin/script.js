...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // API wrappers
  const ProductsAPI = {
    list: () => apiFetch("/api/products").catch(() => safeJson("/Sale/products.json", [])),
  ...
  const OrdersAPI = {
    list: () => apiFetch("/api/orders").catch(() => safeJsonMulti(["/admin/orders.json", "/cart/orderData.json"], [])),
    updateStatus: (id, status) => apiFetch(`/api/orders/${id}`, { method: "PUT", data: { status } })
  };
  const CustomersAPI = {
    list: () => apiFetch("/api/customers").catch(() => safeJson("/admin/customers.json", [])),
  ...
  if (customersTable) {
    customersTable.addEventListener("click", (e) => {
      ...
      if (action === "manage") {
        ...
        const url = `/admin/customer-orders.html?id=${encodeURIComponent(id)}&name=${encodeURIComponent(name)}`;
